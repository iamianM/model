...
class OhlcvEnv(gym.Env):
    ...
    def __init__(self, inputs, window_size, path, show_trade=True, trailing_stop=0.02, initial_portfolio=1000.0, fee=0.0005,
                 margin=25):
        ...
        # load_csv
        self.load_from_csv()
        self.n_features = self.df.shape[1] + 4
...
#         self.df = pd.read_csv(self.path + '5_15_30_1h_timeframes.csv')
        self.train = pd.read_csv(self.path+'timeframes.csv').iloc[:self.inputs.shape[0]]
        self.df = self.train[['close_bid_1min','open_1min','high_1min','low_1min','close_1min']].values.astype(np.float64)

        # self.df.dropna(inplace=True)  # drops Nan rows
        self.closingPrices = self.train['close_1min'].values
    ...
    def step(self, action):
        ...
        self.updateState()
        if self.current_tick > self.inputs.shape[0] - self.window_size - 1:
            self.done = True
    ...
    def get_profit(self):
        if self.position == LONG:
            profit = ((self.closingPrice - self.entry_price) / self.entry_price + 1) * (1 - self.train['spread'].iloc[self.current_tick]) ** 2 - 1
        elif self.position == SHORT:
            profit = ((self.entry_price - self.closingPrice) / self.closingPrice + 1) * (1 - self.train['spread'].iloc[self.current_tick]) ** 2 - 1
        else:
            profit = 0
    ...
    def updateState(self):
        ...
        profit = self.get_profit()
        # append two
        # print([self.inputs[self.current_tick], one_hot_position, [profit]])
        # self.state = [self.inputs[self.current_tick], one_hot_position, [profit]]
