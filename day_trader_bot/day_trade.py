...
def login_webull(email, token_file="token.txt"):
    #login to Webull
    wb = paper_webull()
    try:
        f = open(token_file, "r")
    ...
    if not login_info:
        ...
        wb.refresh_login()
        login_info = wb.login('myemail@email.com', 'my password')

    return wb


def get_hists(path='./daily_stock_data/', index_col='timestamp'):
    hists = {}
    for file in [f for f in listdir(path) if f.endswith('.csv')]:
        try:
            hists[file[:-4]] = pd.read_csv(path+file, index_col=index_col)
...
def candle_score(lst_0, lst_1, lst_2):
    ...
    hammer = (((H_0 - L_0) > 3 * (O_0 - C_0)) & ((C_0 - L_0) / (.001 + H_0 - L_0) > 0.6) & (
                (O_0 - L_0) / (.001 + H_0 - L_0) > 0.6))

    inverted_hammer = (((H_0 - L_0) > 3 * (O_0 - C_0)) & ((H_0 - C_0) / (.001 + H_0 - L_0) > 0.6) & (
                (H_0 - O_0) / (.001 + H_0 - L_0) > 0.6))
    ...
    shooting_Star_bearish = (O_1 < C_1) & (O_0 > C_1) & ((H_0 - max(O_0, C_0)) >= abs(O_0 - C_0) * 3) & (
                (min(C_0, O_0) - L_0) <= abs(O_0 - C_0)) & inverted_hammer

    shooting_Star_bullish = (O_1 > C_1) & (O_0 < C_1) & ((H_0 - max(O_0, C_0)) >= abs(O_0 - C_0) * 3) & (
                (min(C_0, O_0) - L_0) <= abs(O_0 - C_0)) & inverted_hammer
    ...
    Piercing_Line_bullish = (C_1 < O_1) & (C_0 > O_0) & (O_0 < L_1) & (C_0 > C_1) & (C_0 > ((O_1 + C_1) / 2)) & (
                C_0 < O_1)
...
def candle_df(df):
    ...
    for c in range(2, len(df_candle)):
        cscore, cpattern = 0, ''
        lst_2 = [df_candle['open'].iloc[c - 2], df_candle['high'].iloc[c - 2], df_candle['low'].iloc[c - 2],
                 df_candle['close'].iloc[c - 2]]
        lst_1 = [df_candle['open'].iloc[c - 1], df_candle['high'].iloc[c - 1], df_candle['low'].iloc[c - 1],
                 df_candle['close'].iloc[c - 1]]
        lst_0 = [df_candle['open'].iloc[c], df_candle['high'].iloc[c], df_candle['low'].iloc[c],
                 df_candle['close'].iloc[c]]
